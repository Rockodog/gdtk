...
// Symbolic names for solver mode
enum SolverMode {
    transient,
    steady,
    dual_time_stepping,
...
@nogc
string solverModeName(SolverMode sm)
{
    final switch (sm) {
...
SolverMode solverModeFromName(string name)
{
...
string shock_detector_name(ShockDetector sd)
{
    ...
    case ShockDetector.KAD: return "KAD";
    }
}
...
ShockDetector shock_detector_from_name(string name)
{
    ...
    case "KAD": return ShockDetector.KAD;
    default:
    ...
    FEMModelForFSI FEMModelFromName(string name) {
        switch (name) {
            ...
            default:
               ...
               errMsg ~= "\t'eulerBernoulli' \t'kirchoffLove'";
               throw new Error(errMsg);
        }
    }
}
...
struct SolidDomainLooseUpdateOptions {
    ...
    int preconditionerFillIn = 0;
    int preconditionerApproximation = 1;
    bool usePreconditioner = true;
}

final class GlobalConfig {
    shared static SolverMode solverMode = SolverMode.transient;
    //
